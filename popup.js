document.addEventListener('DOMContentLoaded', function() {
  ...
  // 设置扫描详情中的标签文本
  document.getElementById('currentBookmarkLabel').textContent = getMessage('currentBookmarkLabel', '当前书签：');
  document.getElementById('urlLabel').textContent = getMessage('urlLabel', 'URL：');
  document.getElementById('invalidLinksLabel').textContent = getMessage('invalidLinksLabel', '已发现失效链接：');
  // 移除对status元素的引用
  ...
  function restoreScanState() {
    chrome.storage.local.get(['isScanning', 'scanProgress', 'scanTotal', 'currentBookmark', 'invalidLinksCount'], function(result) {
      if (result.isScanning) {
        ...
        // 构建详情对象
        const details = {
          currentIndex: result.currentBookmark && result.scanTotal ? Math.round(result.scanProgress * result.scanTotal / 100) : 0,
